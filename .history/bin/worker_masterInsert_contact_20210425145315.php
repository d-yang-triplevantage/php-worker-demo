...
try{
  //DB接続
   ...
   $sql = 'select a.id as aid,a.sfid as asfid,a.schema as aschema,b.sfid as bsfid,b.schema as bschema,b.lastname as blastname,b.firstname as bfirstname,b.email as bemail,b.phone as bphone from sfdcmiddle.middle_contact a ,sfdcmiddle.middle_contact b
 ...
 // $keyold = ['','',''];
  ...
  //      $keynew = [$firstname,$lastname,$email];
   ...
   //          print('=======keyoldのデータ登録=最後====');
     ...
     //       $lastname=$keynew[1];
     //       $email=$keynew[2];
            $prepIns = $dbh->prepare('INSERT INTO sfdcmaster.master_contact(id,sfid,firstname,lastname,email,phone,vctr__vectorno__c) VALUES(:aid,:bsfid,:firstname,:lastname,:email,:phone,:vctr__vectorno__c)');
            $prepIns->execute(array($aid,$bsfid,$firstname,$lastname,$email,$phone,$vctr__vectorno__c));
           
            //中間テーブルに統合IDを反映
            $sqlUpdate = 'update sfdcmiddle.middle_contact set vctr__vectorno__c=:vctr__vectorno__c WHERE sfid=:bsfid and firstname = :firstname and lastname=:lastname and email=:email and phone=:phone';
            $stmtUpdate = $dbh->prepare($sqlUpdate);
            $stmtUpdate->execute(array($vctr__vectorno__c,$bsfid,$firstname,$lastname,$email,$phone));
