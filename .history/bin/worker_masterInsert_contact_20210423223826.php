 ...
 // $keyold = ['','',''];
  ...
  //      $keynew = [$firstname,$lastname,$email];
   ...
   //          print('=======keyoldのデータ登録=最後====');
     ...
     //       $lastname=$keynew[1];
     //       $email=$keynew[2];
            $prepIns = $dbh->prepare('INSERT INTO sfdcmaster.master_contact(sfid,firstname,lastname,email,phone,vctr__vectorno__c) VALUES(:sfid:firstname,:lastname,:email,:phone,:vctr__vectorno__c)');
            $prepIns->execute(array($bsfid,$firstname,$lastname,$email,$phone,$vctr__vectorno__c));
