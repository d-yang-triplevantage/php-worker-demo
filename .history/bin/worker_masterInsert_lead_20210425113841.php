...
try{
  ...
  $sql = 'select a.id as aid,a.sfid as asfid,a.schema as aschema,b.sfid as bsfid,b.schema as bschema,b.lastname as blastname,b.firstname as bfirstname,b.email as bemail,b.phone as bphone from sfdcmiddle.middle_lead a ,sfdcmiddle.middle_lead b
  ...
  foreach ($stmt as $row) {
    ...
    // }else{
     ...
     //      $lastname=$keynew[1];
     //      $email=$keynew[2];
           $prepIns = $dbh->prepare('INSERT INTO sfdcmaster.master_lead(id,sfid,firstname,lastname,email,phone,vctr__vectorno__c) VALUES(:aid,:asfid,:firstname,:lastname,:email,:phone,:vctr__vectorno__c)');
           $prepIns->execute(array($aid,$asfid,$firstname,$lastname,$email,$phone,$vctr__vectorno__c));

           //中間テーブルに統合IDを反映
          $sqlUpdate = 'update sfdcmiddle.middle_lead set vctr__vectorno__c=:vctr__vectorno__c WHERE sfid = :sfid and firstname = :firstname and lastname = :lastname and email = :email and phone = :phone';
          $stmtUpdate = $dbh->prepare($sqlUpdate);

          $stmtUpdate->bindParam(':vctr__vectorno__c', $vctr__vectorno__c);
          $stmtUpdate->bindParam(':sfid', $asfid);
          $stmtUpdate->bindParam(':firstname', $firstname);
          $stmtUpdate->bindParam(':lastname', $lastname);
          $stmtUpdate->bindParam(':email', $email);
          $stmtUpdate->bindParam(':phone', $phone);
